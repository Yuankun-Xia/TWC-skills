...
materials for the Python track only. Keep the normal `twc-figure` contract first:
...
- Grouped bars: start with `figure_ImmunoStruct`, `figure_CellSpliceNet`, or
  `figure_brainteaser`; then apply the tighter IEEE export and font rules in
  `api.md`.
...
guidance is folded into `twc-figure`:
