...
to find likely IEEE-family candidates, then verify exact journal status on official pages if the
...
- journals beginning with `Nature `, such as `Nature Medicine`, `Nature Biotechnology`,
  `Nature Methods`, `Nature Materials`, `Nature Genetics`, `IEEE journals`
...
- IEEE Xplore, AAAS, and Cell Press official pages should be checked when exact current journal
  coverage matters.
