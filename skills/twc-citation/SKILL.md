...
name: twc-citation
description: >-
  Add strict IEEE-style citations to manuscript text by splitting long passages into citable
  segments, searching IEEE Xplore, flagship communications journals, and related engineering
  venues, filtering by publication time range, and exporting one
  reference-manager-ready output by default. Use this skill whenever the user asks to input text and
  automatically get references, add citations to a paragraph/manuscript, find IEEE-series or CNS
...
- `Nature系列`: search IEEE Xplore first. Include `Nature`, `Nature [field]`,
  `IEEE journals`, `Communications [field]`, `Scientific Reports`, and `npj` journals.
...
- `CNS及其子刊` or `CNS/sister journals`: search only accepted flagship and subjournal titles in
  IEEE Xplore, the AAAS Science family, and Cell Press.
...
- `--scope nature`: IEEE Xplore journals only.
...
检索范围
- [IEEE Xplore / Science family / Cell Press / flagship only, plus date limits]
...
open and browse candidates. If no suitable CNS/IEEE-series paper exists, say so plainly and
...
- Check journal identity. Many journals contain the word "nature" but are not IEEE Xplore journals.
...
| [references/journal-scope.md](references/journal-scope.md) | You need the default IEEE journal-family boundary and official source notes |
...
Crossref REST API and filters, NCBI E-utilities, EndNote RIS import options, IEEE Xplore,
